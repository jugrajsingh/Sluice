...
def test_appspec_defaults_fill_from_name():
    ...
    assert app.storage_prefix == "AppData/topwear"
    assert app.desired_state == "Ready"
    assert app.scaling.max_instances == 0  # 0 = unbounded
    assert app.scaling.min_instances == 0  # no warm floor by default
    assert app.scaling.messages_per_instance == 10
...
def test_placement_is_ordered_discriminated_union():
    ...
    assert app.placement[1].spec.machine_type == "g2"
...
def test_appstatus_defaults():
    ...
    assert s.updated_at == 0.0  # 0 ⇒ never written / unknown (stale signal for readers)
